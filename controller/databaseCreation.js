...
let sql3 = 'CREATE TABLE Token(tokenId int, tokenName VARCHAR(255), tokenURI VARCHAR(255),tokenCreator VARCHAR(255),currentOwner VARCHAR(255),previousOwner VARCHAR(255),transactionHistory VARCHAR(255),tokenDescription VARCHAR(255), tokenPrice int,forSale BOOLEAN,checkApproval VARCHAR(256), PRIMARY KEY (tokenId),CONSTRAINT fk_userToken FOREIGN KEY (currentOwner) REFERENCES User(walletAddress))'; 
