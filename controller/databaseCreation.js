...
let sql3 = 'CREATE TABLE Token(tokenId int AUTO INCREMENT, tokenName VARCHAR(255), tokenURI VARCHAR(255),tokenCreator VARCHAR(255),currentOwner VARCHAR(255),previousOwner VARCHAR(255),transactionHistory VARCHAR(255),tokenDescription VARCHAR(255), tokenPrice int,forSale BOOLEAN, PRIMARY KEY (tokenId),CONSTRAINT fk_userToken FOREIGN KEY (currentOwner) REFERENCES USER(walletAddress)) '; 
